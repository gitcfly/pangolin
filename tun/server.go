...
func NewTunServer(tname string, ipNet string, mtu int) (*TunServer, error) {
	ts := &TunServer{
		...
		InputChan: make(chan []byte, TUNCHANBUFFSIZE),
	}
	...
	go func() {
		...
		for {
			...
			if n, err := ts.TunConn.Read(data); err == nil && n > 0 {
				logging.Log.Debug("Read Data FromTun....")
	...
	go func() {
		...
		for {
			if data, ok := <-ts.InputChan; ok && len(data) > 0 {
				...
				ts.TunConn.Write(data)
			}
		}
	}()
}

func (ts *TunServer) StartClient(client string, inputChan chan []byte, outputChan chan []byte) {
	go func() {
		defer func() {
			recover()
		}()
